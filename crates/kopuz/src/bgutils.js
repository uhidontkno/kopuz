...
function isBrowser() {
  ...
  const hasValidWindow =
    (_c =
        (_b =
            (_a = Object.getOwnPropertyDescriptor(globalThis, "window")) == null
              ? void 0
              : _a.get) == null
          ? void 0
          : _b.toString().includes("[native code]")) != null
      ? _c
      : false;
...
function buildURL(endpointName, useYouTubeAPI) {
  return `${useYouTubeAPI ? YT_BASE_URL : GOOG_BASE_URL}/${
...
async function create(bgConfig, interpreterHash) {
  ...
  if (!response.ok) {
    throw new BGError("[Challenge]: Failed to fetch challenge", {
...
function parseChallengeData(rawData) {
  ...
  const privateDoNotAccessOrElseSafeScriptWrappedValue =
...
var BotGuardClient = class _BotGuardClient {
  ...
  async load() {
    ...
    const vmFunctionsCallback = (
      ...
      Object.assign(this.vmFunctions, {
        asyncSnapshotFunction,
        shutdownFunction,
        passEventFunction,
        checkCameraFunction,
    ...
    try {
      ...
      throw new BGError(
        `[BotGuardClient]: Failed to load program (${error.message})`,
      );
    }
    return this;
  }
  ...
  async snapshot(args) {
    return new Promise((resolve, reject) => {
      if (!this.vmFunctions.asyncSnapshotFunction) {
        return reject(
          new BGError("[BotGuardClient]: Async snapshot function not found"),
        );
      }
...
var WebPoMinter = class _WebPoMinter {
  ...
  static async create(integrityTokenResponse, webPoSignalOutput) {
    ...
    if (!integrityTokenResponse.integrityToken) {
      throw new BGError(
        "Failed to create WebPoMinter: No integrity token provided",
        integrityTokenResponse,
      );
    }
    const mintCallback = await getMinter(
      base64ToU8(integrityTokenResponse.integrityToken),
    );
  ...
  async mint(identifier) {
    const result = await this.mintCallback(
...
async function generate(args) {
  ...
  const integrityTokenResponse = await bgConfig.fetch(
    buildURL("GenerateIT", bgConfig.useYouTubeAPI),
    {
      method: "POST",
      headers: getHeaders(),
      body: JSON.stringify(payload),
...
function decodeColdStartToken(token) {
  ...
  if (packet.length !== totalPacketLength) {
    throw new BGError("Invalid packet length.", {
      packetLength: packet.length,
      expectedLength: totalPacketLength,
    });
  }
  const payload = packet.subarray(2);
  const keyLength = 2;
  ...
  const timestamp = payload[4] << 24 | payload[5] << 16 | payload[6] << 8 |
    payload[7];
